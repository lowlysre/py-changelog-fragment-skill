...
That's also why one skill covers three tools instead of shipping as three separate skills: an agent working across several Python projects hits `antsibull-changelog` in one, `towncrier` in another, `scriv` in a third, all solving the same problem with just enough surface difference (fragment naming, config location, category handling) to trip up a generic instruction. A single skill gives a consistent entry point, detect the tool, then defer to its specifics, so switching projects doesn't mean switching mental models.
